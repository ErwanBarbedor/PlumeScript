 ...
 function(plume) 
 	function plume._run(chunk,arguments) 
 		...
 		function ACC_TEXT(vm,arg1,arg2) 
 			...
 			for i=start,stop do
 			if _STACK_GET(vm.mainStack,i) ==vm.empty then _STACK_SET(vm.mainStack,i, "" )
			end
		end
 		local acc_text=table.concat(vm.mainStack, "" ,start,stop)_STACK_MOVE(vm.mainStack,start)_STACK_SET(vm.mainStack,start,acc_text)_END_ACC(vm)
	end
 	...
 	for i=1, #t,3 do
 	if t[i+2]then _TABLE_META_SET(vm,args,t[i] ,t[i+1] ) 
 		else _TABLE_SET(vm,args,t[i] ,t[i+1] )
		end
	end _STACK_MOVE(vm.mainStack,limit-2)_STACK_PUSH(vm.mainStack,args)_END_ACC(vm)
end
 function ACC_CHECK_TEXT(vm,arg1,arg2) 
 	...
 	if t~= "number"and t~= "string"and value~=vm.empty then
 		if t== "table"and value.meta.table.tostring then
 			...
 			local args= { }_STACK_SET(vm.mainStack,_STACK_POS(vm.mainStack) ,_CALL(vm,meta,args) ) 
 			else _ERROR(vm, "Cannot concat a '".. t .."' value." )
			end
		end
	end
 	function _END_ACC(vm)_STACK_POP(vm.mainStack.frames)
	end
 	function ACC_EMPTY(vm,arg1,arg2)_STACK_PUSH(vm.mainStack,vm.empty)_END_ACC(vm)
	end
end
 do
 			...
 			if tleft== "table"and left.meta and left.meta.table[name .."r" ]then
 				...
 				elseif tright== "table"and right.meta and right.meta.table[name .."l" ]then
 					...
 					function _BIN_OPP_BOOL(vm,opp) 
 						...
 						left=_CHECK_BOOL(vm,left)_STACK_PUSH(vm.mainStack,opp(right,left) )
					end
 					function _UN_OPP_BOOL(vm,opp) 
 						local x=_STACK_POP(vm.mainStack) 
 						x=_CHECK_BOOL(vm,x)_STACK_PUSH(vm.mainStack,opp(x) )
					end
 					function _BIN_OPP_NUMBER(vm,opp,name) 
 						...
 						if lerr or rerr then
 							...
 							if success then _STACK_PUSH(vm.mainStack,result) 
 								else _ERROR(vm,lerr or rerr)
								end
							end
 							function _UN_OPP_NUMBER(vm,opp,name) 
 								...
 								if err then
 									...
 									function OPP_ADD(vm,arg1,arg2)_BIN_OPP_NUMBER(vm,_ADD, "add" )
									end
 									function OPP_MUL(vm,arg1,arg2)_BIN_OPP_NUMBER(vm,_MUL, "mul" )
									end
 									function OPP_SUB(vm,arg1,arg2)_BIN_OPP_NUMBER(vm,_SUB, "sub" )
									end
 									function OPP_DIV(vm,arg1,arg2)_BIN_OPP_NUMBER(vm,_DIV, "div" )
									end
 									function OPP_MOD(vm,arg1,arg2)_BIN_OPP_NUMBER(vm,_MOD, "mod" )
									end
 									function OPP_POW(vm,arg1,arg2)_BIN_OPP_NUMBER(vm,_POW, "pow" )
									end
 									function OPP_NEG(vm,arg1,arg2)_UN_OPP_NUMBER(vm,_NEG, "minus" )
									end
 									function _AND(x,y) 
 										return x and y
									end
 									...
 									function OPP_AND(vm,arg1,arg2)_BIN_OPP_BOOL(vm,_AND)
									end
 									function OPP_OR(vm,arg1,arg2)_BIN_OPP_BOOL(vm,_OR)
									end
 									function OPP_NOT(vm,arg1,arg2)_UN_OPP_BOOL(vm,_NOT)
									end
 									function _LT(x,y) 
 										return x<y
									end
 									function OPP_LT(vm,arg1,arg2)_BIN_OPP_NUMBER(vm,_LT, "lt" )
									end
 									function OPP_EQ(vm,arg1,arg2) 
										...
										end _STACK_PUSH(vm.mainStack,result)
									end
								end
 								do
 								function ACC_CALL(vm,arg1,arg2) 
 									...
 									if t== "macro"then
 										...
 										local arguments= { }_UNSTACK_POS(vm,tocall,arguments,capture)_UNSTACK_NAMED(vm,tocall,arguments,capture) 
 										if self then table.insert(arguments,self)
										end
 										if tocall.variadicOffset>0 then
 											arguments[tocall.variadicOffset] =capture
										end _END_ACC(vm)_STACK_PUSH(vm.mainStack,_CALL(vm,tocall,arguments) ) 
 										elseif t== "luaFunction"then ACC_TABLE(vm)table.insert(vm.chunk.callstack, {chunk=vm.chunk,macro=tocall,ip=vm.ip} ) 
 											local success,result=pcall(tocall.callable,_STACK_GET(vm.mainStack) ,vm.chunk) 
 											if success then table.remove(vm.chunk.callstack) 
 												if result==nil then
 													result=vm.empty
												end _STACK_POP(vm.mainStack)_STACK_PUSH(vm.mainStack,result) 
 												else _ERROR(vm,result)
												end
 												else _ERROR(vm, "Try to call a '".. t .."' value" )
												end
											end
 											function _UNSTACK_POS(vm,macro,arguments,capture) 
 												...
 												if argcount~=macro.positionalParamCount and macro.variadicOffset==0 then
 													...
 													if not name then
 														name=macro.name or"???"
													end _ERROR(vm, "Wrong number of positionnal arguments for macro '".. name .."', ".. argcount .." instead of ".. macro.positionalParamCount .."." )
												end
 												for i=1,macro.positionalParamCount do
 												arguments[i] =_STACK_GET_OFFSET(vm.mainStack,i-argcount)
											end
 											for i=macro.positionalParamCount+1,argcount do table.insert(capture.table,_STACK_GET_OFFSET(vm.mainStack,i-argcount) )
										end _STACK_MOVE_FRAMED(vm.mainStack)
									end
 									function _UNSTACK_NAMED(vm,macro,arguments,capture) 
 										...
 										if m then _TABLE_META_SET(vm,capture,k,v) 
 											elseif j then
 												arguments[j] =v
 												elseif macro.variadicOffset>0 then _TABLE_SET(vm,capture,k,v) 
 													else
 														local name=macro.name or"???" 
 														err= "Unknow named parameter '".. k .."' for macro '".. name .."'."
													end
												end
 												if err then _ERROR(vm,err) 
 													else _STACK_POP(vm.mainStack)
													end
												end
 												function _CALL(vm,macro,arguments)table.insert(vm.chunk.callstack, {chunk=vm.chunk,macro=macro,ip=vm.ip} ) 
 													if#vm.chunk.callstack<=1000 then
 														local success,callResult,cip,source=vm.plume.run(macro,arguments) 
 														if success then table.remove(vm.chunk.callstack) 
 															return callResult
 															else _SPECIAL_ERROR(vm,callResult,cip, (source or macro) )
															end
 															else _ERROR(vm, "stack overflow" )
															end
														end
													end
 													do
 													...
 													function _VM_INIT(plume,chunk,arguments)require"table.new" 
 														local vm= { } 
 														vm.plume=plume _VM_INIT_VARS(vm,chunk)_VM_INIT_ARGUMENTS(vm,chunk,arguments) 
 														return vm
													end
 													...
 													function _VM_INIT_ARGUMENTS(vm,chunk,arguments) 
 														if arguments then
 															if chunk.isFile then
 																...
 																for i=1,chunk.local sCount do
 																if arguments[i] ==nil then _STACK_SET(vm.variableStack,i,vm.empty) 
 																	else _STACK_SET(vm.variableStack,i,arguments[i] )
																	end
																end _STACK_MOVE(vm.variableStack,chunk.local sCount)_STACK_PUSH(vm.variableStack.frames,1)
															end
														end
													end
 													...
 													function GET_ITER(vm,arg1,arg2) 
 															...
 															if obj.meta.table.next then
 																...
 																if iter.type== "luaFunction"then
 																	...
 																	elseif iter.type== "table"then
																		...
																		end _STACK_PUSH(vm.mainStack,value) 
 																		else _ERROR(vm, "Try to iterate over a non-table '".. tobj .."' value." )
																		end
																	end
 																	function FOR_ITER(vm,arg1,arg2) 
 																		...
 																		if iter.type== "luaFunction"then
 																			...
 																			if result==vm.empty then JUMP(vm,0,arg2) 
 																				else _STACK_PUSH(vm.mainStack,result)
																				end
																			end
																		end
 																		do
 																	...
 																	do
 																	function LOAD_CONSTANT(vm,arg1,arg2)_STACK_PUSH(vm.mainStack,vm.constants[arg2] )
																	end
 																	function LOAD_LEXICAL(vm,arg1,arg2)_STACK_PUSH(vm.mainStack,_STACK_GET_FRAMED(vm.variableStack,arg2-1, -arg1) )
																	end
 																	function LOAD_LOCAL(vm,arg1,arg2)_STACK_PUSH(vm.mainStack,_STACK_GET_FRAMED(vm.variableStack,arg2-1) )
																	end
 																	function LOAD_STATIC(vm,arg1,arg2)_STACK_PUSH(vm.mainStack,vm.static[arg2] )
																	end
 																	function LOAD_TRUE(vm,arg1,arg2)_STACK_PUSH(vm.mainStack,true)
																	end
 																	function LOAD_FALSE(vm,arg1,arg2)_STACK_PUSH(vm.mainStack,false)
																	end
 																	function LOAD_EMPTY(vm,arg1,arg2)_STACK_PUSH(vm.mainStack,vm.empty)
																	end
																end
 																do
 														...
 														function SWITCH(vm,arg1,arg2) 
 															local x=_STACK_POP(vm.mainStack) 
 															local y=_STACK_POP(vm.mainStack)_STACK_PUSH(vm.mainStack,x)_STACK_PUSH(vm.mainStack,y)
														end
 														function DUPLICATE(vm,arg1,arg2)_STACK_PUSH(vm.mainStack,_STACK_GET(vm.mainStack) )
														end
 														function NULL(vm,arg1,arg2)
														end
													end
 													do
 													function ENTER_SCOPE(vm,arg1,arg2)_STACK_PUSH(vm.variableStack.frames,_STACK_POS(vm.variableStack) +1-arg1) 
 														for i=1,arg2-arg1 do _STACK_PUSH(vm.variableStack,vm.empty)
													end
												end
 												function LEAVE_SCOPE(vm,arg1,arg2)_STACK_POP_FRAME(vm.variableStack)
												end
											end
 											do
 											...
 											function _STACK_MOVE_FRAMED(stack)_STACK_MOVE(stack,_STACK_GET(stack.frames) )
											end
 											function _STACK_POP_FRAME(stack)_STACK_MOVE(stack,_STACK_POP(stack.frames) -1)
											end
 											function _STACK_SET_FRAMED(stack,offset,frameOffset,value)_STACK_SET(stack,_STACK_GET_OFFSET(stack.frames,frameOffset or 0) + (offset or 0) ,value)
											end
 										...
 										do
 										function STORE_LOCAL(vm,arg1,arg2)_STACK_SET_FRAMED(vm.variableStack,arg2-1,0,_STACK_POP(vm.mainStack) )
										end
 										function STORE_STATIC(vm,arg1,arg2) 
 											vm.static[arg2] =_STACK_POP(vm.mainStack)
										end
 										function STORE_LEXICAL(vm,arg1,arg2)_STACK_SET_FRAMED(vm.variableStack,arg2-1, -arg1,_STACK_POP(vm.mainStack) )
										end
 										function STORE_VOID(vm,arg1,arg2)_STACK_POP(vm.mainStack)
										end
									end
 									do
 									function TABLE_NEW(vm,arg1,arg2)_STACK_PUSH(vm.mainStack,table.new(0,arg1) )
									end
 									function _TABLE_SET(vm,t,k,v) 
 										local key=k
 										local value=v
 										key=tonumber(key)or key
 										if not t.table[key]then table.insert(t.keys,k)
										end
 										t.table[key] =value
									end
 									function TABLE_SET_ACC(vm,arg1,arg2) 
 										local t=_STACK_GET_FRAMED(vm.mainStack)table.insert(t,_STACK_POP(vm.mainStack) )table.insert(t,_STACK_POP(vm.mainStack) )table.insert(t,arg2==1)
									end
 									...
 									function TABLE_INDEX(vm,arg1,arg2) 
 										...
 										if key==vm.empty then
 											if arg1==1 then LOAD_EMPTY(vm) 
 												else _ERROR(vm, "Cannot use empty as key." )
												end
 												else
 													local tt=_GET_TYPE(vm,t) 
 													if tt~= "table"then
 														if arg1==1 then LOAD_EMPTY(vm) 
 															else _ERROR(vm, "Try to index a '".. tt .."' value." )
															end
 															else
 																local value=t.table[key] 
 																if value then _STACK_PUSH(vm.mainStack,value) 
 																	else
 																		if arg1==1 then LOAD_EMPTY(vm) 
 																			elseif t.meta.table.getindex then
 																				local meta=t.meta.table.getindex
 																				local args= {key}_STACK_PUSH(vm.mainStack,_CALL(vm,meta,args) ) 
 																				else
 																					if tonumber(key)then _ERROR(vm, "Invalid index '".. key .."'." ) 
 																						else _ERROR(vm, "Unregistered key '".. key .."'." )
																						end
																					end
																				end
																			end
 																	...
 																	function TABLE_INDEX_ACC_SELF(vm,arg1,arg2) 
 																		local t=_STACK_GET_FRAMED(vm.mainStack)table.insert(t, "self" )table.insert(t,_STACK_GET(vm.mainStack) )table.insert(t,false)TABLE_INDEX(vm,0,0)
																	end
 																	...
 																	function _TABLE_META_SET(vm,t,k,v) 
 																		...
 																		function TABLE_SET(vm,arg1,arg2) 
 																			...
 																			local key=_STACK_POP(vm.mainStack) 
 																			local value=_STACK_POP(vm.mainStack) 
 																			if not t.table[key]then table.insert(t.keys,key) 
 																		...
 																		function TABLE_EXPAND(vm,arg1,arg2) 
 																			...
 																			local tt=_GET_TYPE(vm,t) 
 																			if tt== "table"then
 																				for _,item in ipairs(t.table)do _STACK_PUSH(vm.mainStack,item)
																			end
 																			local ft=_STACK_GET_FRAMED(vm.mainStack) 
 																			for _,key in ipairs(t.keys)do table.insert(ft,key)table.insert(ft,t.table[key] )table.insert(ft,false)
																		end
 																		else _ERROR(vm, "Try to expand a '".. tt .."' value." )
																		end
																	end
																end
 																do
 																																																																...
 																																																																elseif op==50 then
 																																																																	...
 																																																																	elseif op==51 then
 																																																																		...
 																																																																		elseif op==52 then
 																																																																		 ...
 																																																																		 ::LOAD_CONSTANT::LOAD_CONSTANT(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::LOAD_TRUE::LOAD_TRUE(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::LOAD_FALSE::LOAD_FALSE(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::LOAD_EMPTY::LOAD_EMPTY(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::LOAD_LOCAL::LOAD_LOCAL(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::LOAD_LEXICAL::LOAD_LEXICAL(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::LOAD_STATIC::LOAD_STATIC(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::STORE_LOCAL::STORE_LOCAL(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::STORE_LEXICAL::STORE_LEXICAL(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::STORE_STATIC::STORE_STATIC(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::STORE_VOID::STORE_VOID(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::TABLE_NEW::TABLE_NEW(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::TABLE_ADD::TABLE_ADD(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::TABLE_SET::TABLE_SET(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::TABLE_INDEX::TABLE_INDEX(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::TABLE_INDEX_ACC_SELF::TABLE_INDEX_ACC_SELF(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::TABLE_SET_META::TABLE_SET_META(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::TABLE_INDEX_META::TABLE_INDEX_META(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::TABLE_SET_ACC::TABLE_SET_ACC(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::TABLE_SET_ACC_META::TABLE_SET_ACC_META(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::TABLE_EXPAND::TABLE_EXPAND(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::ENTER_SCOPE::ENTER_SCOPE(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::LEAVE_SCOPE::LEAVE_SCOPE(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::BEGIN_ACC::_STACK_PUSH(vm.mainStack.frames,vm.mainStack.pointer+1) 
 																																																																		goto DISPATCH
 																																																																		 ::ACC_TABLE::ACC_TABLE(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::ACC_TEXT::ACC_TEXT(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::ACC_EMPTY::ACC_EMPTY(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::ACC_CALL::ACC_CALL(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::ACC_CHECK_TEXT::ACC_CHECK_TEXT(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::JUMP_IF::JUMP_IF(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::JUMP_IF_NOT::JUMP_IF_NOT(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::JUMP_IF_NOT_EMPTY::JUMP_IF_NOT_EMPTY(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::JUMP::JUMP(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::JUMP_IF_PEEK::JUMP_IF_PEEK(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::JUMP_IF_NOT_PEEK::JUMP_IF_NOT_PEEK(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::GET_ITER::GET_ITER(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::FOR_ITER::FOR_ITER(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_ADD::OPP_ADD(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_MUL::OPP_MUL(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_SUB::OPP_SUB(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_DIV::OPP_DIV(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_NEG::OPP_NEG(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_MOD::OPP_MOD(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_POW::OPP_POW(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_LT::OPP_LT(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_EQ::OPP_EQ(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_AND::OPP_AND(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_NOT::OPP_NOT(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::OPP_OR::OPP_OR(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::DUPLICATE::DUPLICATE(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::SWITCH::SWITCH(vm,arg1,arg2) 
 																																																																		goto DISPATCH
 																																																																		 ::END:: 
